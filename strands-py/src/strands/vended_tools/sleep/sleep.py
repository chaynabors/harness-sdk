...
cap). Sleeps are implemented with :func:`asyncio.sleep`, so cancelling the
enclosing task raises :class:`asyncio.CancelledError` and the tool re-raises it
without swallowing.
"""
...
def make_sleep(
    *,
    ...
    :func:`asyncio.sleep`. Cancellation of the surrounding task propagates as
    :class:`asyncio.CancelledError` so agent cancellation aborts the sleep
