...
- Reasoning events expose the following keys:
    - **`reasoning_text`**: Text from the reasoning process
    - **`reasoning_signature`**: Signature from the reasoning process
    - **`reasoning_redacted_content`**: Reasoning content redacted by the model
- **Deprecated reasoning keys** (still emitted for backwards compatibility, removed in a future major release):
    - **`reasoning`**: `True` for reasoning events; prefer matching on the `reasoning_text` /
      `reasoning_signature` / `reasoning_redacted_content` keys directly
    - **`reasoningText`**: alias for `reasoning_text`
    - **`reasoningRedactedContent`**: alias for `reasoning_redacted_content`
</Tab>
<Tab label="TypeScript">
